...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    if argv and not any(arg.startswith("--") for arg in argv):
        coerced = []
        if len(argv) % 2 != 0:
            raise ValueError("Positional arguments must be provided as key value pairs.")
...
def compute_metrics(
    ...
    ref_sets = [list(ref_set) for ref_set in zip(*refs)] if refs else []
    bleu = bleu_metric.corpus_score(hyps, ref_sets).score / 100 if refs else 0.0
